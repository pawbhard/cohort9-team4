...
void SwitchDataBuffer::startDataCollection(void) {
    ...
    while(collect == true) {
        //Do SNMP get 
        if ( doSnmpGet("CISCO-PROCESS-MIB::cpmCPUTotal1minRev.1", switch_ip.c_str(), &data) == SUC) {
        ...
        if(doSnmpGet("CISCO-PROCESS-MIB::cpmCPUMemoryUsed.1", switch_ip.c_str(), &data) == SUC) {
